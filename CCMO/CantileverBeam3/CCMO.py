...
def environmental_selection(pop, pop_size):
    frontno, maxfront = nd_sort(pop.objs, pop_size)
...
def cal_fitness(fitness_objs, fitness_cons=None):
    N = fitness_objs.shape[0]
    dominate = np.zeros((N, N), dtype=bool)

    if fitness_cons is None:
        fitness_cons = np.zeros((N, 1))

    for i in range(0, N - 1):
        for j in range(i + 1, N):
            if fitness_cons[i] == 0 and fitness_cons[j] == 1:
                dominate[i][j] = True
            elif fitness_cons[i] == 1 and fitness_cons[j] == 0:
                dominate[j][i] = True
            else:
                k = int(np.any(fitness_objs[i] < fitness_objs[j])) - int(
                    np.any(fitness_objs[i] > fitness_objs[j])
                )
                if k == 1:
                    dominate[i][j] = True
                elif k == -1:
    ...
    distance = cdist(fitness_objs, fitness_objs)
...
def OperatorGAhalf(ParentDec, lb, ub):
    ...
    Site = np.random.random((N, D // 2)) < proM / (D // 2)
...
def enviromnent_selection(enviromnent_pop, pop_size, is_origin):
    if is_origin:
        fitness = cal_fitness(enviromnent_pop.objs, enviromnent_pop.cons)
    else:
        fitness = cal_fitness(enviromnent_pop.objs)

    next = fitness < 1
    if np.sum(next) < pop_size:
        rank = np.argsort(fitness)
        next[rank[:pop_size]] = True
    elif np.sum(next) > pop_size:
        delete = truncation(enviromnent_pop[next].objs, np.sum(next) - pop_size)
        temp = np.argwhere(next)
        next[temp[delete]] = False

    enviromnent_pop = enviromnent_pop[next]
    fitness = fitness[next]

    ind = np.argsort(fitness)
    enviromnent_pop = enviromnent_pop[ind]
    fitness = fitness[ind]

    return enviromnent_pop, fitness
...
if __name__ == '__main__':
    ...
    Dec1 = np.random.uniform(np.tile(lb, (pop_size, 1)), np.tile(ub, (pop_size, 1)))
    Dec1 = np.round(Dec1).astype(int)

    Dec2 = np.random.uniform(np.tile(lb, (pop_size, 1)), np.tile(ub, (pop_size, 1)))
    Dec2 = np.round(Dec2).astype(int)

    Pop1 = Population(decs=Dec1)
    Pop2 = Population(decs=Dec2)

    Pop1 = Cal(Pop1, modelDatabase, cellsize, row, col, indexToElLabel)
    Pop2 = Cal(Pop2, modelDatabase, cellsize, row, col, indexToElLabel)

    fitness1 = cal_fitness(Pop1.objs, Pop1.cons)
    fitness2 = cal_fitness(Pop2.objs)
    ...
    while Gen <= MaxGen:
        ...
        MatingPool1 = tournament_selection(2, pop_size, fitness1)
        MatingPool2 = tournament_selection(2, pop_size, fitness2)

        OffDec1 = OperatorGAhalf(Pop1[MatingPool1].decs, lb, ub)
        OffDec2 = OperatorGAhalf(Pop1[MatingPool2].decs, lb, ub)

        Offspring1 = Population(decs=OffDec1)
        Offspring1 = Cal(Offspring1, modelDatabase, cellsize, row, col, indexToElLabel)
        Offspring2 = Population(decs=OffDec2)
        Offspring2 = Cal(Offspring2, modelDatabase, cellsize, row, col, indexToElLabel)

        temp1 = Pop1 + Offspring1 + Offspring2
        Pop1, fitness1 = enviromnent_selection(temp1, pop_size, True)

        temp2 = Pop2 + Offspring1 + Offspring2
        Pop2, fitness2 = enviromnent_selection(temp2, pop_size, False)
