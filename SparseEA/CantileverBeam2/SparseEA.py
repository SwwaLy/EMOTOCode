...
def Operator(ParentDec, Fitness):
    # Parameter setting
    N, D = ParentDec.shape
    Parent1Dec = ParentDec[: int(N / 2), :]
    Parent2Dec = ParentDec[int(N / 2):, :]
    # Crossover for dec
    ...
    for i in range(int(N / 2)):
        if np.random.random(1) < 0.5:
            temp_index = np.argwhere(
                np.logical_and(
                    Parent1Dec[i, :], np.logical_not(Parent2Dec[i, :])
                )
                != 0
            )
            ...
            if len(index) == 0:
                ...
                index = index[TS(-Fitness.flatten()[index])]
                OffDec[i, index] = 0
        else:
            temp_index = np.argwhere(
                np.logical_and(
                    np.logical_not(Parent1Dec[i, :]), Parent2Dec[i, :]
                )
                != 0
            )
            index = temp_index.flatten()
    ...
    for i in range(int(N / 2)):
        if np.random.random(1) < 0.5:
            ...
            index = np.argwhere(np.logical_not(OffDec[i, :]) != 0)
            if len(index) == 0:
                OffDec
            else:
                index = index[TS(Fitness.flatten()[index])]
...
def TS(Fitness):
    if len(Fitness) == 0:
        ...
        index = tournament_selection(2, 1, Fitness)
    return index
...
def EnvironmentalSelection(Pop, Dec, N):
    # Delete duplicated solutions
    _, uni = np.unique(Pop.objs, return_index=True, axis=0)
    Pop = Pop[uni]
    Dec = Dec[uni, :]
    N = np.minimum(N, Pop.decs.shape[0])
    # Non-dominated sorting
    FrontNo, MaxFNo = nd_sort(Pop.objs, Pop.cons, N)
    Next = FrontNo < MaxFNo
    # Calculate the crowding distance of each solution
    CrowdDis = crowding_distance(Pop.objs, FrontNo)
    ...
    Dec = Dec[Next, :]
    return Pop, Dec, FrontNo, CrowdDis
...
if __name__ == '__main__':
    ...
    col = 150
    row = 50
    minExcavation = int(nrElements * 0.3)
    nvar = nrElements
    Fitness = np.random.permutation(7500) + 1
    Dec = np.ones((pop_size, nvar))
    for i in range(pop_size):
        Dec[i, tournament_selection(2, minExcavation, Fitness)] = 0
    Pop = Population(decs=Dec)
    Pop = Cal(Pop, modelDatabase, row, col, indexToElLabel)
    Pop, Dec, FrontNo, CrowdDis = EnvironmentalSelection(Pop, Dec, pop_size)
    ...
    while Gen <= MaxGen:
        ...
        MatingPool = tournament_selection(2, 2 * pop_size, FrontNo, -CrowdDis)
        OffDec = Operator(Dec[MatingPool, :], Fitness)
        Offspring = Population(decs=OffDec)
        Offspring = Cal(Offspring, modelDatabase, row, col, indexToElLabel)
        Pop, Dec, FrontNo, CrowdDis = EnvironmentalSelection(Pop + Offspring, np.vstack((Dec, OffDec)), pop_size)
